<template>
    <div class='px-3 overflow-hidden relative z-10'>
        <el-scrollbar ref='scrollbar' wrap-class='scrollbar-wrapper'>
            <div class='layout-tags-container flex items-center'>
                <span
                    v-for='v in tagsList'
                    :key='v.path'
                    :ref='getTagsDom'
                    class='border border-gray-200 px-2 py-1 mx-1 cursor-pointer rounded-[4px] flex items-center flex-nowrap flex-shrink-0'
                ...
                >
                    ...
                    <router-link :to='v.path' :class='v.isActive ? &apos;text-[#FEFEFE]&apos; : &apos;text-[#666]&apos;'>{{ v.title
                    }}</router-link>
    ...
    watch(
        ...
        () => nextTick(() => {
            ...
            nextTick(() => {
                ...
                if (!scrollbar.value) return
                const scrollLeft = itemWidth - scrollbar.value.wrapRef.offsetWidth + 70
                if (scrollLeft > 0) scrollbar.value.wrapRef.scrollLeft = scrollLeft
            })
        })
    )
    // 确保在每次变更之前重置引用
